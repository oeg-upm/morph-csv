...
def projectCSVfiles(csvColumns,all_columns):
    all_columns= dict({"source":"file.csv","columns":["tax_id","GeneID","Symbol","LocusTag","Synonyms","dbXrefs","chromosome","map_location","description","type_of_gene","Symbol_from_nomenclature_authority","Full_name_from_nomenclature_authority","Nomenclature_status","Other_designations","Modification_date","Feature_type"]})
    csvColumns = dict({"source":"file.csv","columns":["tax_id","GeneID","Symbol","LocusTag","Synonyms","dbXrefs","Full_name_from_nomenclature_authority","Modification_date","Feature_type"]})

    for tm in csvColumns:
        select_columns = csvColumns["columns"]
        origin_columns = all_columns["columns"]
        source = csvColumns["source"] 
        source = ("./tmp/csv/" + source)

    os.system('bash ./bash/filtercsv.sh \'%s\' \'%s\' %s'%(delimiter,arg,path))
